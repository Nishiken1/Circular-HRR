...
def generate_circular_vectors(num_vectors, dims):
    """
    Generate n vectors of size dims that are orthogonal to each other.
    """
    if num_vectors > dims:
    ...
    # Inticalizing class vectors.
    # vecs = torch.randn(dims, num_vectors, dtype=torch.float)
    circular_vecs = (torch.rand(dims, num_vectors, dtype=torch.float) - 0.5) * 2 * torch.pi
    #circular_vecs = (torch.rand(dims, num_vectors, dtype=torch.float)) * torch.pi

    # print(circular_vecs.shape)
    # Using QR decomposition to get orthogonal vectors.
    vecs, _ = torch.qr(circular_vecs)
    vecs = vecs.t()
    # print(vecs.shape)

    return vecs
    # return circular_vecs.t() 
...
def gen_rand_circular_vec(dims):
    """
    Generate a random vector of size dims.
    """
    return np.random.uniform(low=-np.pi, high=np.pi, size=dims)
    #return np.random.uniform(low=0, high=np.pi, size=dims)


def non_normalized_gen_rand_vec(dims):
    
    return (np.random.normal(0, 1. / dims, size=(dims)))
...
def get_vectors(num_vectors, dims, ortho=False):
    if ortho:
        ...
        #vectors = [non_normalized_gen_rand_vec(dims) for i in range(num_vectors)]
        return torch.from_numpy(np.array(vectors, dtype=np.float32))
    
def get_circular_vectors(num_vectors, dims, ortho=False):
    if ortho:
        vectors = generate_circular_vectors(num_vectors, dims)
        return vectors
    else:
        vectors = [gen_rand_circular_vec(dims) for i in range(num_vectors)]
        return torch.from_numpy(np.array(vectors, dtype=np.float32))
