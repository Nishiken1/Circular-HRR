...
class SPNDataset(Dataset):
    def __init__(self, data_file, dimension_reduction=False):
        ...
        self.features = []
        self.labels = []
        with open(self._file_loc, "rb") as fptr:
        ...
        self.tfidf_features = self.load_tfidf_features(self._file_loc)

    def load_tfidf_features(self, tfidf_file):
        # npzファイルからTF-IDFデータを読み込む
        with np.load(tfidf_file, allow_pickle=True) as data:
    ...
    def __getitem__(self, idx):
        if torch.is_tensor(idx):
            idx = idx.tolist()
        feature = self.tfidf_features[idx].toarray()
...
class SPNSparseDataset(Dataset):
    ...
    def __len__(self):
        # データセットの長さを返す方法を更新する
        return self.tfidf_features.shape[0]
    ...
    def get_max_size(self):
        if isinstance(self.labels, list):
            ...
            return self.labels.shape[1]  # NumPy配列の場合
    ...
    def __init__(self, data_file, dimension_reduction=False):
        ...
        with open(self._file_loc, "rb") as fptr:
            ...
            for row_id, row in enumerate(fptr):
                ...
                except Exception as e:  # この部分がエラーハンドリングのコードです
                    print(f"Error while parsing labels at row {row_id}: {e}")
                    continue
                try:
                    # Extract features.
                    for f in data[1:]:
                    ...
                    print(f"Error while parsing features at row {row_id}: {e}")
                    continue
